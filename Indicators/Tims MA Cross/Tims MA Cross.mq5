...
const int            BaselineBufferIndex = 0;
const int            HighBufferIndex     = 1;
const int            MidBufferIndex      = 2;
...
int                  OnInit() {
   ...
   SlowMAHandle   = iMA( Symbol(), Period(), InpBaselineBars, 0, InpBaselineMethod, PRICE_CLOSE );
   FastMAHandle   = iMA( Symbol(), Period(), InpFastBars, 0, InpFastMethod, PRICE_CLOSE );
...
void OnDeinit( const int reason ) {
   ...
   for ( int k = 0; k <= 20; k++ ) {
      ObjectDelete( 0, StringFormat( "Take Profit %i", k ) );
      ObjectDelete( 0, StringFormat( "Stop Loss %i", k ) );
   }
}
...
int OnCalculate( const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[],
   ...
   if ( rates_total < InpBaselineBars - 1 || rates_total < InpFastBars - 1 || rates_total < InpATRBars - 1 || rates_total < InpVolBars - 1 ) {
      return ( 0 );
   }
   ...
   CopyBuffer( SlowMAHandle, 0, 0, limit, BaselineBuffer );
   CopyBuffer( FastMAHandle, 0, 0, limit, MidBuffer );

   CopyBuffer( ATRHandle, 0, 0, limit, ATRBuffer );
...
double Indicator( int handle, int shift ) {

   double buf[];
   ArraySetAsSeries( buf, true );
   CopyBuffer( handle, 0, 0, shift + 1, buf );
   IndicatorRelease( handle );

   return ( buf[shift] );
}

double   Open( int i ) { return ( iOpen( Symbol(), Period(), i ) ); }
double   Close( int i ) { return ( iClose( Symbol(), Period(), i ) ); }
double   High( int i ) { return ( iHigh( Symbol(), Period(), i ) ); }
double   Low( int i ) { return ( iLow( Symbol(), Period(), i ) ); }
datetime Time( int i ) { return ( iTime( Symbol(), Period(), i ) ); }
long     Volume( int i ) { return ( iVolume( Symbol(), Period(), i ) ); }
